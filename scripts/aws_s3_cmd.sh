...
_aws_s3_bucket_list_cmd() {
	...
	aws s3api list-buckets "${list_args[@]}" --output json |
		jq -r "$bucket_list_jq" | awk -v styles="bold,faint" -f "$_aws_s3_cmd_source_dir/aws_render.awk"
}
...
_aws_s3_object_list_cmd() {
	...
	aws s3api list-objects-v2 --bucket "$bucket" --max-items "${FZF_AWS_S3_MAX_ITEMS:-1000}" "${list_args[@]}" --output json |
		jq -r "$object_list_jq" | awk -v styles="bold,normal,faint,faint" -f "$_aws_s3_cmd_source_dir/aws_render.awk"
}
